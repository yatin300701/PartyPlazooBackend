...
class CommentServices {
    static async addComments(payload) {
        let find = await DbServices_1.DataBase.commentsCollection.findOne({ userId: new mongodb_1.ObjectId(payload.userId) });
        if (!find) {
    ...
    static async allReples(id) {
        let result = await DbServices_1.DataBase.replyCollection.find({ commentId: new mongodb_1.ObjectId(id) }).toArray();
        return result;
    }
